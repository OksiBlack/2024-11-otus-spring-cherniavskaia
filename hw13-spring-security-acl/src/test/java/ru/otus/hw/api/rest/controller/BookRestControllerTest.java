...
class BookRestControllerTest {
    ...
    void testGetById_NotFound() throws Exception {
        ...
        // Act & Assert: Perform the request and assert the expected outcomes
        mockMvc.perform(get("/api/books/1"))
            .andExpect(status().isNotFound()) // Expect 404 Not Found status
            .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON)) // Expect JSON content type
            .andExpect(jsonPath("$.title").value("Not Found")) // Check for the title in the problem details
            .andExpect(jsonPath("$.detail").value("Book not found")); // Check the detail message
    }
